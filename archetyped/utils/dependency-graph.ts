...
export class DependencyGraph {
  ...
  constructor(deps: ExtensionDefinition[]) {
    this.deps = deps.map((dep: ExtensionDefinition, index: number) => {
      ...
      dep_.provides.forEach(
        (provided: string) => (this.dependencyIds[provided] = index)
      );
      return dep_;
    });
  }
  ...
  resolve(): { extensions: ExtensionDefinition[]; error?: string } {
    ...
    for (const dep of Object.keys(this.graph)) {
      if (this.resolved.has(dep)) {
        continue;
      }
      const result = this.resolveRecur(dep);
      if (!result.resolved) return { extensions: [], error: result.message };
    }
    const orderedIds: Set<number> = Array.from(this.resolved).reduce(
      (prev, curr) => {
  ...
  buildGraph(): GraphResolution {
    ...
    for (const dep of this.deps) {
      for (const provided of dep.provides!) {
        // Fail if duplicate
  ...
  resolveIter(config: ExtensionDefinition[], lookup?: Function) {
    const extensions: Dependency[] = config.map(
      (extensionConfig: ExtensionDefinition, index: number) => {
        return {
          ...extensionConfig,
          id: index,
        };
    ...
    while (extensions.length && changed) {
      ...
      extensions_.forEach((extension: Dependency) => {
        const consumes = [...extension.consumes!];
    ...
    if (extensions.length) {
      const unresolved_: { [key: string]: string[] | null } = {};
      ...
      const unresolved: { [key: string]: string[] } = Object.keys(unresolved_)
